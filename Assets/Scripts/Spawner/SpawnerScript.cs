...
public class SpawnerScript : MonoBehaviour
{
    ...
    public Rigidbody Enemy = null; //Enemy (e.g Bug), that we want the spawner to spawn.
    ...
    public float x_pos, z_pos; //Modifiable float, that can change the spawn distance from the spawn centre.
    ...
    private void InstantiateEnemy()
    {
        ...
        Instantiate(Enemy, new Vector3(Random.Range(SpawnerPos.x - x_pos, SpawnerPos.x + x_pos),
                                              y, //Default y-coordinate spawn for enemy.
                                              Random.Range(SpawnerPos.z - z_pos, SpawnerPos.z + z_pos)), this.gameObject.transform.rotation);
    }
}
